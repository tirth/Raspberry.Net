...
namespace Raspberry.IO.GeneralPurpose.Behaviors
{
    ...
    public class ChaserBehavior : PinsBehavior
    {
        ...
        private bool Overflow => Loop && !RoundTrip;

        private int MinimumStep => Overflow ? 0 : WidthBefore;

        private int MaximumStep => Configurations.Length - 1 - (Overflow ? 0 : WidthAfter);

        private int WidthBefore => (Width%2) == 1 ? (Width - 1)/2 : Width/2;

        private int WidthAfter => (Width%2) == 1 ? (Width - 1)/2 : Width/2 - 1;
